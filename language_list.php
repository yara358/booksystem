<!-- this code will show all the lanuages we have in the 
...
<?php
	...
	$query = "SELECT * FROM languages ORDER BY language_id";
	...
	// counting how much publisher have books inside the database
		while($row = mysqli_fetch_assoc($result)){
			...
			while ($pubInBook = mysqli_fetch_assoc($result2)){
				if($pubInBook['languages'] == $row['language_id']){
					$count++;
				}
			}
	?>
		<li><!--the customer can enter the page that have full books wanted of every publisher -->
		    ...
		    <a href="bookPerPub.php?pubid=<?php echo $row['language_id']; ?>"><?php echo $row['language_name']; ?></a>
		</li>
	<?php } ?>
		<li>
